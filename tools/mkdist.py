...
def walk_kconfig(SFW_ROOT, source_list):
    for parent, dirnames, filenames in os.walk(SFW_ROOT):
        if 'bsp' in parent:
            continue
        if '.git' in parent:
            continue
...
def bsp_copy_files(bsp_root, dist_dir):
    ...
    do_copy_folder(os.path.join(bsp_root), dist_dir,
        ignore_patterns('build', 'dist', '*.pyc', '*.old', '*.map', 'sfw.bin', '.sconsign.dblite', '*.elf', '*.axf', 'cconfig.h'))
...
def bsp_update_sconstruct(dist_dir):
    ...
    with open(os.path.join(dist_dir, 'SConstruct'), 'w') as f:
        for line in data:
            if line.find('SFW_ROOT') != -1:
                if line.find('sys.path') != -1:
                    f.write('# set SFW_ROOT\n')
                    f.write('if not os.getenv("SFW_ROOT"): \n    SFW_ROOT="sfw"\n\n')
            f.write(line)

def bsp_update_kconfig(dist_dir):
    # change SFW_ROOT in Kconfig
...
def bs_update_ide_project(bsp_root, sfw_root, sfwide = None):
    import subprocess
    ...
    if sfwide == None:
        ...
        item = 'eclipse --project-name=' + sfwide['project_name']
        tgt_dict = {item:('gcc', 'gcc')}

    scons_env = os.environ.copy()
    scons_env['SFW_ROOT'] = sfw_root
...
def MkDist_Strip(program, BSP_ROOT, SFW_ROOT, Env):
    global source_list
    ...
    target_list = []
    libcpu_dir = os.path.join(SFW_ROOT, 'libcpu').lower()
    libc_dir = os.path.join(SFW_ROOT, 'components', 'libc', 'compilers').lower()
    sal_dir = os.path.join(SFW_ROOT, 'components', 'net', 'sal_socket').lower()
    ...
    for src in source_list:
        ...
        if src.lower().startswith(SFW_ROOT.lower()):
    ...
    walk_kconfig(SFW_ROOT, source_list)

    # copy all files to target directory
    source_list.sort()
    for src in source_list:
        dst = src.replace(SFW_ROOT, '')
    ...
    print('=> tools')
    do_copy_folder(os.path.join(SFW_ROOT, 'tools'), os.path.join(target_path, 'tools'), ignore_patterns('*.pyc'))
    do_copy_file(os.path.join(SFW_ROOT, 'Kconfig'), os.path.join(target_path, 'Kconfig'))
    do_copy_file(os.path.join(SFW_ROOT, 'AUTHORS'), os.path.join(target_path, 'AUTHORS'))
    do_copy_file(os.path.join(SFW_ROOT, 'COPYING'), os.path.join(target_path, 'COPYING'))
    do_copy_file(os.path.join(SFW_ROOT, 'README.md'), os.path.join(target_path, 'README.md'))
    do_copy_file(os.path.join(SFW_ROOT, 'README_zh.md'), os.path.join(target_path, 'README_zh.md'))

    print('=> %s' % os.path.join('components', 'libc', 'compilers'))
    do_copy_folder(os.path.join(SFW_ROOT, 'components', 'libc', 'compilers'), os.path.join(target_path, 'components', 'libc', 'compilers'))

    if sources_include_sal:
        print('=> %s' % os.path.join('components', 'net', 'sal_socket'))
        do_copy_folder(os.path.join(SFW_ROOT, 'components', 'net', 'sal_socket'), os.path.join(target_path, 'components', 'net', 'sal_socket'))

    # copy all libcpu/ARCH directory
    import sfwprofile
    print('=> %s' % (os.path.join('libcpu', sfwprofile.ARCH, sfwprofile.CPU)))
    do_copy_folder(os.path.join(SFW_ROOT, 'libcpu', sfwprofile.ARCH, sfwprofile.CPU), os.path.join(target_path, 'libcpu', sfwprofile.ARCH, sfwprofile.CPU))
    if os.path.exists(os.path.join(SFW_ROOT, 'libcpu', sfwprofile.ARCH, 'common')):
        print('=> %s' % (os.path.join('libcpu', sfwprofile.ARCH, 'common')))
        do_copy_folder(os.path.join(SFW_ROOT, 'libcpu', sfwprofile.ARCH, 'common'), os.path.join(target_path, 'libcpu', sfwprofile.ARCH, 'common'))
    do_copy_file(os.path.join(SFW_ROOT, 'libcpu', 'Kconfig'), os.path.join(target_path, 'libcpu', 'Kconfig'))
    do_copy_file(os.path.join(SFW_ROOT, 'libcpu', 'SConscript'), os.path.join(target_path, 'libcpu', 'SConscript'))

    # change SFW_ROOT in SConstruct
...
def MkDist(program, BSP_ROOT, SFW_ROOT, Env, sfwide = None):
    ...
    # copy tools directory
    print('=> components')
    do_copy_folder(os.path.join(SFW_ROOT, 'components'), os.path.join(target_path, 'components'))
    ...
    print('=> include')
    do_copy_folder(os.path.join(SFW_ROOT, 'include'), os.path.join(target_path, 'include'))

    # copy all libcpu/ARCH directory
    print('=> libcpu')
    import sfwprofile
    do_copy_folder(os.path.join(SFW_ROOT, 'libcpu', sfwprofile.ARCH), os.path.join(target_path, 'libcpu', sfwprofile.ARCH))
    do_copy_file(os.path.join(SFW_ROOT, 'libcpu', 'Kconfig'), os.path.join(target_path, 'libcpu', 'Kconfig'))
    do_copy_file(os.path.join(SFW_ROOT, 'libcpu', 'SConscript'), os.path.join(target_path, 'libcpu', 'SConscript'))

    # copy src directory
    print('=> src')
    do_copy_folder(os.path.join(SFW_ROOT, 'src'), os.path.join(target_path, 'src'))
    ...
    if sfwide == None:
        ...
        bs_update_ide_project(dist_dir, target_path, sfwide)
