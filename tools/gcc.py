...
def GetGCCRoot(sfwprofile):
    exec_path = sfwprofile.EXEC_PATH
    prefix = sfwprofile.PREFIX
...
def CheckHeader(sfwprofile, filename):
    root = GetGCCRoot(sfwprofile)
...
def GetNewLibVersion(sfwprofile):
    version = 'unknown'
    root = GetGCCRoot(sfwprofile)

    if CheckHeader(sfwprofile, '_newlib_version.h'): # get version from _newlib_version.h file
        ...
        if f:
            ...
            f.close()
    elif CheckHeader(sfwprofile, 'newlib.h'): # get version from newlib.h
...
def GCCResult(sfwprofile, str):
    ...
    gcc_cmd = os.path.join(sfwprofile.EXEC_PATH, sfwprofile.CC)
