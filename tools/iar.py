...
def IARWorkspace(target):
    ...
    xml = iar_workspace % target.replace('iar', '')
    out.write(xml)
    out.close()

    # remove project.ewd file
    project_ewd = os.path.abspath(target).replace('.ewp', '.ewd')
    if os.path.isfile(project_ewd):
        os.unlink(project_ewd)
    # copy template.ewd and rename it as sfw.ewd
    if os.path.exists('iar\\template.ewd'):
        shutil.copy2('iar\\template.ewd', 'iar\\sfw.ewd')

def IARProject(target, script):
    project_path = os.path.dirname(os.path.abspath(target))
    tree = etree.parse(project_path + '\\template.ewp')
    root = tree.getroot()
    ...
    for group in script:
        ...
        # get path for assembler
        if 'ASPATH' in group and group['ASPATH']:
            ASPATH += group['ASPATH']
    ...
    for option in options:
        ...
        if name.text == 'IlinkAdditionalLibs':
            for path in Libs:
                state = SubElement(option, 'state')
                if os.path.isabs(path) or path.startswith('$'):
                    path = path #path.decode(fs_encoding)
                else:
                    path = '$PROJ_DIR$\\' + path #('$PROJ_DIR$\\' + path).decode(fs_encoding)
                state.text = path
...
def IARVersion():
    ...
    def IARPath():
        ...
        os.environ['SFW_CC'] = 'iar'
        utils.ReloadModule(sfwprofile)

        # get iar path
        path = sfwprofile.EXEC_PATH
    ...
    if os.path.exists(path):
        ...
        print('Error: get IAR version failed. Please update the IAR installation path in sfwprofile.py!')
        exit(-1)
