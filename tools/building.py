...
def PrepareBuilding(env, root_directory, has_libcpu=False, remove_components = []):
    ...
    AddOption('--genconfig',
                ...
                action = 'store_true',
                default = False,
                help = 'Generate .config from sfwconfig.h')
    AddOption('--useconfig',
                dest = 'useconfig',
                type = 'string',
                help = 'make sfwconfig.h from config file.')
    AddOption('--verbose',
                dest = 'verbose',
                action = 'store_true',
                default = False,
    ...
    Env = env
    sfw_root = os.path.abspath(root_directory)
    ...
    if ide_name:
        SetOption('no_exec', 1)
        try:
            sfwprofile.CROSS_TOOL, sfwprofile.PLATFORM = ide_dict[ide_name]
            # replace the 'SFW_CC' to 'CROSS_TOOL'
            os.environ['SFW_CC'] = sfwprofile.CROSS_TOOL
            utils.ReloadModule(sfwprofile)
        except KeyError:
        ...
        and sfwprofile.PLATFORM == 'gcc':
        AddDepend('CONFIG_MINILIBC')

    # auto change the 'SFW_EXEC_PATH' when 'sfwprofile.EXEC_PATH' get failed
    if not os.path.exists(sfwprofile.EXEC_PATH):
        if 'SFW_EXEC_PATH' in os.environ:
            # del the 'SFW_EXEC_PATH' and using the 'EXEC_PATH' setting on sfwprofile.py
            del os.environ['SFW_EXEC_PATH']
            utils.ReloadModule(sfwprofile)

    # add compability with Keil MDK 4.6 which changes the directory of armcc.exe
    if sfwprofile.PLATFORM == 'armcc' or sfwprofile.PLATFORM == 'armclang':
        if sfwprofile.PLATFORM == 'armcc' and not os.path.isfile(os.path.join(sfwprofile.EXEC_PATH, 'armcc.exe')):
            if sfwprofile.EXEC_PATH.find('bin40') > 0:
                sfwprofile.EXEC_PATH = sfwprofile.EXEC_PATH.replace('bin40', 'armcc/bin')
    ...
    if env['PLATFORM'] == 'win32':
        os.environ['PATH'] = sfwprofile.EXEC_PATH + ";" + os.environ['PATH']
    else:
        os.environ['PATH'] = sfwprofile.EXEC_PATH + ":" + os.environ['PATH']

    # add program path
    env.PrependENVPath('PATH', os.environ['PATH'])
    # add sfwconfig.h/BSP path into source group
    DefineGroup("source", [], [], CPPPATH=[str(Dir('#').abspath)], ASPATH=[str(Dir('#').abspath)])
    ...
    # parse sfwconfig.h to get used component
    PreProcessor = PatchedPreProcessor()
    f = open('sfwconfig.h', 'r')
    contents = f.read()
    f.close()
    ...
    if sfwprofile.PLATFORM == 'gcc' and str(env['LINKFLAGS']).find('nano.specs') != -1:
    ...
    AddOption('--menuconfig',
                ...
                action = 'store_true',
                default = False,
                help = 'make menuconfig for SFW platform')
    ...
    if configfn:
        from menuconfig import mk_sfwconfig
        mk_sfwconfig(configfn)
        exit(0)
    ...
    bsp_vdir = 'build'
    boot_vdir = 'build/firmware'
    # board build script
    objs = SConscript('SConscript', variant_dir=bsp_vdir, duplicate=0)
    # include source
    objs.extend(SConscript(sfw_root + '/firmware/SConscript', variant_dir=boot_vdir, duplicate=0))

    # include components
    objs.extend(SConscript(sfw_root + '/component/SConscript',
...
def PrepareModuleBuilding(env, root_directory, bsp_directory):
    ...
    Env = env
    sfw_root = root_directory

    # parse bsp sfwconfig.h to get used component
    PreProcessor = PatchedPreProcessor()
    f = open(bsp_directory + '/sfwconfig.h', 'r')
    contents = f.read()
    f.close()
    PreProcessor.process_contents(contents)
    BuildOptions = PreProcessor.cpp_namespace
...
def DoBuilding(target, objects):
    ...
    Env.AddPreAction(target, sfwprofile.PRE_ACTION)
...
def GenTargetProject(program = None):
    ...
    if GetOption('ide') == 'mdk4':
        from keil import MDK4Project
        MDK4Project('sfw.uvproj', Projects)

    if GetOption('ide') == 'mdk5':
        from keil import MDK5Project
        MDK5Project('mdk\sfw.uvprojx', Projects)

    if GetOption('ide') == 'iar':
        from iar import IARProject
        IARProject('iar\sfw.ewp', Projects)
...
def EndBuilding(target, program = None):
    ...
    if hasattr(sfwprofile, 'BSP_LIBRARY_TYPE'):
        Env['bsp_lib_type'] = sfwprofile.BSP_LIBRARY_TYPE

    if hasattr(sfwprofile, 'dist_handle'):
        Env['dist_handle'] = sfwprofile.dist_handle

    Env.AddPostAction(target, sfwprofile.POST_ACTION)
    ...
    if GetOption('make-dist') and program != None:
        ...
        MkDist(program, BSP_ROOT, sfw_root, Env)
        need_exit = True
    if GetOption('make-dist-strip') and program != None:
        from mkdist import MkDist_Strip
        MkDist_Strip(program, BSP_ROOT, sfw_root, Env)
        need_exit = True
    if GetOption('make-dist-ide') and program != None:
        ...
        sfw_ide = {'project_path' : project_path, 'project_name' : project_name}
        MkDist(program, BSP_ROOT, sfw_root, Env, sfw_ide)
        need_exit = True

    if not GetOption('help') and not GetOption('ide'):
        if not os.path.exists(sfwprofile.EXEC_PATH):
            print ("Error: the toolchain path (" + sfwprofile.EXEC_PATH + ") is not exist, please check 'EXEC_PATH' in path or sfwprofile.py.")
            need_exit = True
...
def GetVersion():
    ...
    rtdef = os.path.join(sfw_root, 'include', 'sfwdef.h')

    # parse sfwdef.h to get SFW version
    ...
    version = int(filter(lambda ch: ch in '0123456789.', def_ns['SFW_VERSION']))
    subversion = int(filter(lambda ch: ch in '0123456789.', def_ns['SFW_SUBVERSION']))

    if 'SFW_REVISION' in def_ns:
        revision = int(filter(lambda ch: ch in '0123456789.', def_ns['SFW_REVISION']))
