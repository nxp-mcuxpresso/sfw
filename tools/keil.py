...
def MDK5Project(target, script):
    project_path = os.path.dirname(os.path.abspath(target))
    template_tree = etree.parse(project_path + '\\template.uvprojx')
    ...
    if os.path.isfile(project_uvopt):
        os.unlink(project_uvopt)
    # copy uvopt file
    if os.path.exists('mdk\\template.uvoptx'):
        import shutil
        shutil.copy2('mdk\\template.uvoptx', 'mdk\\sfw.uvoptx')
...
def ARMCC_Version():
    ...
    if os.path.exists(path):
        ...
        print('Error: get armcc version failed. Please update the KEIL MDK installation path in sfwprofile.py!')
        return "0.0"
