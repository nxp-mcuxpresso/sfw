...
{
    status_t status;
    status = sfw_flash_read(addr, buffer, length);
    return status;
}
...
static int32_t mflash_drv_sector_update(uint32_t sector_addr, uint32_t sect_off, const uint8_t *data, uint32_t data_len)
{
    ...
    uint32_t page_program_map[(MFLASH_SECTOR_SIZE / MFLASH_PAGE_SIZE) / 32] = {0}; /* Current implementation max page number is (MFLASH_SECTOR_SIZE / MFLASH_PAGE_SIZE) pages per sector */
    ...
    for (uint32_t i = 0; i < data_len; i++)
    {
        ...
        if (cur_value != new_value)
        {
            ...
            page_program_map[((sect_off + i) / MFLASH_PAGE_SIZE) / 32] |= 1 << (((sect_off + i) / MFLASH_PAGE_SIZE) % 32);
        }
    }
    ...
    for (int page_idx = 0; (0 == sector_erase_req) && page_idx < MFLASH_SECTOR_SIZE / MFLASH_PAGE_SIZE; page_idx++)
    {
        ...
        if (page_program_map[page_idx / 32] & (1 << (page_idx % 32)))
        {
        ...
        for (int page_idx = 0; page_idx < MFLASH_SECTOR_SIZE / MFLASH_PAGE_SIZE; page_idx++)
        {
            ...
            for (int i = page_word_start; i < page_word_end; i++)
            {
                if (g_flashm_sector[i] != 0xFFFFFFFF)
                {
                    ...
                    page_program_map[page_idx / 32] |= (1 << (page_idx % 32));
                    break;
                }
            }
        }
