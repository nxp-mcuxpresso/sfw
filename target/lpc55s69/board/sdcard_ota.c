...
status_t DEMO_SDCardOTAUpgrade(void)
{
    ...
    sfw_flash_read(FLASH_AREA_IMAGE_1_OFFSET + IMAGE_VERSION_OFFSET, version_buff, 8);
    ...
    status = sfw_flash_erase(FLASH_AREA_IMAGE_2_OFFSET, FLASH_AREA_IMAGE_2_SIZE);
    if (status) 
    {
        goto exit;
    }
    ...
    {  
        ...
        {
            primask = DisableGlobalIRQ();
            status = sfw_flash_write(dstAddr, (uint32_t *)g_bufferRead, sizeof(g_bufferRead));
            if (status) 
            {
                goto exit;
            }
