...
BaseType_t mflash_read_file(char *pcFileName, uint8_t **ppucData, uint32_t *pulDataSize)
{
    ...
    if (0 == tmp_file.flash_addr)
        return pdFALSE;
    /* Ontain pointer to meta data */
    //tmp_meta = (mfile_meta_t *)mflash_drv_mmap(tmp_file.flash_addr, sizeof(mfile_meta_t));
    tmp_meta = (mfile_meta_t *)(tmp_file.flash_addr + MFLASH_BASE_ADDRESS);
    ...
    *pulDataSize = tmp_meta->file_size;
    ...
    //*ppucData = (uint8_t *)mflash_drv_mmap(tmp_file.flash_addr + sizeof(mfile_meta_t), tmp_meta->file_size);
    *ppucData = (uint8_t *)(tmp_file.flash_addr + sizeof(mfile_meta_t) + MFLASH_BASE_ADDRESS);
    if (NULL == *ppucData)
        return pdFALSE;
