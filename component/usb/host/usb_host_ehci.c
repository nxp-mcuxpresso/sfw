                                 ...
                                 void *startEntryPointer)
{
    ...
    else
    {
         ...
         * 2) */
        if ((distance <= (USB_HOST_EHCI_ISO_BOUNCE_FRAME_NUMBER + frameInterval * 2U)) && (distance > 0U))
        {
                                              ...
                                              usb_host_transfer_t *transfer)
{
    ...
    if (ehciInstance->ehciSitdNumber >= sitdNumber)
    {
        ...
        {
            /*misra 10.8*/
            tempIndex = (((usb_host_ehci_sitd_t *)sitdPointer->nextLinkPointer) - ehciInstance->ehciSitdIndexBase);
                                            ...
                                            uint16_t uframeInterval)
{
    ...
    if (lastLinkUframe != 0xFFFFU)
    {
         ...
         * * 2) */
        if ((distance <= ((uint32_t)USB_HOST_EHCI_ISO_BOUNCE_UFRAME_NUMBER + ((uint32_t)uframeInterval * 8U))) &&
            (distance > 2U))
        {
                                            ...
                                            usb_host_ehci_itd_t *endItdPointer)
{
    ...
    while (1U == 1U)
    {
        ...
        /* release itd */
        /* USB_HostEhciLock(); */
        USB_HostEhciZeroMem((void *)itdPointer, sizeof(usb_host_ehci_itd_t) >> 2);
...
#error "Please increase the instance count."
    ...
    {
        ehciInstance->ehciItdList[index - 1U].nextLinkPointer = (uint32_t)(&ehciInstance->ehciItdList[index]);
    }
    ehciInstance->ehciItdNumber                                                  = USB_HOST_CONFIG_EHCI_MAX_ITD;
    ehciInstance->ehciItdList[USB_HOST_CONFIG_EHCI_MAX_ITD - 1U].nextLinkPointer = 0U;
#endif /* USB_HOST_CONFIG_EHCI_MAX_ITD */
