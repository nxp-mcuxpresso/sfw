...
typedef struct _pd_board_chip_instance
{
    ...
    pd_hpd_detect_t hpdDetectInstance;
...
pd_status_t PD_DpBoardChipsInit(void **interfaceHandle, pd_handle pdHandle, const void *param)
{
    ...
    if (PD_DpHpdDetectInit(&pdChipInstance->hpdDetectInstance, pdChipInstance->pdHandle,
                           ((const pd_dp_peripheral_config_t *)param)->hpdDetectConfig) != 0)
    {
    ...
    {
        PD_DpHpdDetectDeinit(&pdChipInstance->hpdDetectInstance);
        PD_CrossbarCBTLDeinit(&pdChipInstance->crossbarInstance);
        pdChipInstance->occupied = 0;
        return kStatus_PD_Error;
    }
...
pd_status_t PD_DpBoardChipsControl(void *interfaceHandle, uint32_t opCode, void *opParam)
{
    ...
    switch (opCode)
    {
        ...
        case kDPPeripheal_ControlHPDDetectStart:
            PD_DpHpdDetectStart(&pdChipInstance->hpdDetectInstance);
            break;

        case kDPPeripheal_ControlHPDDetectStop:
            PD_DpHpdDetectStop(&pdChipInstance->hpdDetectInstance);
            break;

        case kDPPeripheal_ControlHPDQueueEnable:
            PD_DpHpdModeEnterEnableDetectQueue(&pdChipInstance->hpdDetectInstance);
            break;

        case kDPPeripheal_ControlHPDQueueDisable:
            PD_DpHpdModeExitClearDetectQueue(&pdChipInstance->hpdDetectInstance);
            break;

        case kDPPeripheal_ControlHPDGetCurrentState:
            if (PD_DpHpdDetectGetPinValue(&pdChipInstance->hpdDetectInstance))
            {
                *((uint8_t *)opParam) = kDPHPDDetect_High;
            }
            else
            {
                *((uint8_t *)opParam) = kDPHPDDetect_Low;
            }
            break;
...
void PD_DpBoardChip1msIsr(void)
{
    for (uint8_t index = 0; index < sizeof(s_PDBoardChipInstances) / sizeof(pd_board_chip_instance_t); ++index)
    {
        ...
        {
            PD_DpHpdDetect1msISR(&s_PDBoardChipInstances[index].hpdDetectInstance);
        }
    }
}
