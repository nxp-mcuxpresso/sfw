...
#define SHIELD_BOARD_VBUS_DECREASE_FROM_9V_TIME (100)
#define SHIELD_BOARD_VBUS_INCREASE_TO_9V_TIME (30)
...
pd_status_t PD_PowerBoardReset(uint8_t port)
{
    ...
    /* Disable extra SRC */
    HAL_GpioSetOutput(powerControl->powerGpioHandle, 1);
#endif
...
pd_status_t PD_PowerBoardSourceEnableVbusPower(uint8_t port, pd_vbus_power_t vbusPower)
{
    ...
    if (vbusPower.minVoltage > VSAFE5V_IN_50MV)
    {
        ...
        HAL_GpioSetOutput(powerControl->powerGpioHandle, 0); /* Enable extra SRC */
...
#if (defined BOARD_PD_EXTERNAL_POWER_SUPPORT) && (BOARD_PD_EXTERNAL_POWER_SUPPORT)
        HAL_GpioSetOutput(powerControl->powerGpioHandle, 1); /* Disable extra SRC */
