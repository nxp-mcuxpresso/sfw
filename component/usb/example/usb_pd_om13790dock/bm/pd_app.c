...
#define ptn36502_I2cWriteRegister(i2cHandle, slaveAddress, registerAdd, data) \
    PD_I2cWriteBlocking(i2cHandle, slaveAddress, registerAdd, 1, (data), 1);
#endif
...
pd_status_t PD_DpmConnectCallback(void *callbackParam, uint32_t event, void *param)
{
    ...
    switch (event)
    {
        ...
        case PD_CONNECTED:
        {
            ...
            HAL_GpioSetOutput((hal_gpio_handle_t)(&pdAppInstance->gpioOrientHandle[0]), (getInfo ? 1 : 0));
...
#if (defined PD_CONFIG_PHY_LOW_POWER_LEVEL) && (PD_CONFIG_PHY_LOW_POWER_LEVEL)
/* Return 0 - all ports are in low power state, Non-zero - at least one of port is not in low power state. */
...
/* Disable peripheral before entering low power state. */
void PD_DemoEnterLowPower(void)
{
    HAL_TimerDisable((hal_timer_handle_t)&g_PDTimerHandle[0]);
    DbgConsole_Deinit();
}

/* Re-enable peripheral after exiting low power state. */
void PD_DemoExitLowPower(void)
{
