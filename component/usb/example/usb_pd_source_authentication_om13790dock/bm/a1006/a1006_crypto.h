 ...
 * internally.
 *
 * @param pubKey    Pointer to public key used to precompute the response
 */
void a100x_precomputeResponse(const uint8_t *pubKey);

/*!
 * @brief Compares A100x response to challenge with precomputed response.
 * Before comparing, the precomputed response is converted from affine to the randomized projective coordinate.
 *
 * @param buf       Pointer to A100x response to challenge
 * @param bufSize   Size of response
 *
