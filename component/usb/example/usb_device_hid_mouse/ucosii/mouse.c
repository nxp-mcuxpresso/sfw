...
void APP_task(void *handle)
{
#if defined(USB_DEVICE_CONFIG_USE_TASK) && (USB_DEVICE_CONFIG_USE_TASK > 0)
    uint8_t error;
#if defined(OS_CRITICAL_METHOD) && (OS_CRITICAL_METHOD == 3u)
    OS_CPU_SR cpu_sr = 0u;
#endif
#endif
    ...
    {
        if (OSTaskCreateExt(USB_DeviceTask,                   /* pointer to the task */
                            g_UsbDeviceHidMouse.deviceHandle, /* Task input parameter */
                            &g_UsbDeviceHidMouse.deviceTask.stackMemory[UCOSII_DEVICE_TASK_STACK / sizeof(OS_STK) - 1U],
                            UCOSII_DEVICE_TASK_PRIORITY,                /* Task priority */
                            UCOSII_DEVICE_TASK_PRIORITY,                /* Task id. should be the same as priority */
                            g_UsbDeviceHidMouse.deviceTask.stackMemory, /* Task stack bottom */
                            UCOSII_DEVICE_TASK_STACK / sizeof(OS_STK),  /* Task stack size */
                            (void *)0U,                                 /* TCB extension not supported */
                            OS_TASK_OPT_NONE                            /* optional task handle to create */
                            ) != OS_ERR_NONE)
        {
        ...
        OSTaskNameSet(UCOSII_DEVICE_TASK_PRIORITY, (INT8U *)"USB device task", &error);
#endif

        OS_ENTER_CRITICAL();
        g_UsbDeviceHidMouse.deviceTask.taskHandle = OSTCBPrioTbl[UCOSII_DEVICE_TASK_PRIORITY];
        OS_EXIT_CRITICAL();
    }
#endif
...
#endif
{
    ...
    g_UsbDeviceHidMouse.applicationTask.stackMemory = (OS_STK *)&g_UsbDeviceHidMouse.applicationTaskStack[0];
    g_UsbDeviceHidMouse.deviceTask.stackMemory      = (OS_STK *)&g_UsbDeviceHidMouse.deviceTaskStack[0];

    if (OSTaskCreateExt(
            APP_task,             /* pointer to the task */
            &g_UsbDeviceHidMouse, /* Task input parameter */
            &g_UsbDeviceHidMouse.applicationTask.stackMemory[UCOSII_APPLICATION_TASK_STACK / sizeof(OS_STK) - 1U],
            UCOSII_APPLICATION_TASK_PRIORITY,                /* Task priority */
            UCOSII_APPLICATION_TASK_PRIORITY,                /* Task id. should be the same as priority */
            g_UsbDeviceHidMouse.applicationTask.stackMemory, /* Task stack bottom */
            UCOSII_APPLICATION_TASK_STACK / sizeof(OS_STK),  /* Task stack size */
            (void *)0U,                                      /* TCB extension not supported */
            OS_TASK_OPT_NONE                                 /* optional task handle to create */
            ) != OS_ERR_NONE)
    {
...
#if OS_TASK_NAME_EN > 0u
    OSTaskNameSet(UCOSII_APPLICATION_TASK_PRIORITY, (INT8U *)"USB device mouse", &error);
#endif

    OS_ENTER_CRITICAL();
    g_UsbDeviceHidMouse.applicationTask.taskHandle = OSTCBPrioTbl[UCOSII_APPLICATION_TASK_PRIORITY];
    OS_EXIT_CRITICAL();
