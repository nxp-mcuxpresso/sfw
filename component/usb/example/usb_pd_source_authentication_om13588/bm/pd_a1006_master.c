...
extern void PD_DemoPDTransferPolling(pd_app_t *pdAppInstance);
                         ...
                         uint8_t *cert)
{
    ...
    pdAppInstance->transfering = 1;
    /* read A1006 compressed cert */
    if (PD_I2cOverVdmRead(pdAppInstance->i2cHandle, interfaceAddr, A1006_COMPRESSED_CERT_OFFSET, (ifBufAndUID + 18),
                          130))
    {
        PRINTF("I2C error\r\n");
        return 1;
    }
    while (pdAppInstance->transfering)
    {
        PD_DemoPDTransferPolling(pdAppInstance);
    }
                         ...
                         uint8_t *ifBuf)
{
    ...
    pdAppInstance->transfering = 1;
    /* send challenge */
    if (PD_I2cOverVdmWrite(pdAppInstance->i2cHandle, interfaceAddr, A1006_CHALLENGE_OFFSET, ifBuf, 0x2C))
    {
        PRINTF("I2C error\r\n");
        return 1;
    }
    ...
    {
        pdAppInstance->transfering = 1;
        if (PD_I2cOverVdmRead(pdAppInstance->i2cHandle, interfaceAddr, A1006_CHALLENGE_RESPONSE_OFFSET, ifBuf, 46))
        {
            return 1;
        }
        while (pdAppInstance->transfering)
...
uint8_t PD_A1006MasterInit(pd_app_t *pdAppInstance)
{
    ...
    PD_I2cInit(&s_A1006I2cHandle, kInterface_i2c0 + BOARD_PD_I2C_INDEX, HW_I2CGetFreq, HW_I2CReleaseBus);
...
uint8_t PD_A1006MasterAuth(pd_app_t *pdAppInstance)
{
    uint8_t fail = 0;
    const uint32_t *rootPub;
    uint8_t interfaceAddr = I2C_SLAVE_ADDR_SHIELD1;
    ...
    pdAppInstance->transfering = 1;
    /* get uid */
    fail |= PD_I2cOverVdmRead(pdAppInstance->i2cHandle, interfaceAddr, A1006_UID_OFFSET, g_ifBuf, 18);
    if (fail)
    {
        return fail;
    }
    ...
    if ((pdAppInstance->transferResult) || g_ifBuf[0] || g_ifBuf[1])
    {
        /* try another I2C address */
        interfaceAddr              = I2C_SLAVE_ADDR_SHIELD2;
        pdAppInstance->transfering = 1;
        fail |= PD_I2cOverVdmRead(pdAppInstance->i2cHandle, interfaceAddr, A1006_UID_OFFSET, g_ifBuf, 18);
        if (fail)
        {
    ...
    fail |= a100x_host_cert(pdAppInstance, interfaceAddr, CERT_SLOT, rootPub, g_ifBuf, cert);
    if (fail)
    {
        return fail;
    }
    fail |= a100x_host_auth(pdAppInstance, interfaceAddr, (uint8_t *)HOST_UID, (sizeof(HOST_UID) - 1),
...
void PD_DemoPDTransferPolling(pd_app_t *pdAppInstance)
{
    PD_InstanceTask(pdAppInstance->pdHandle);
    PD_I2cOverVdmTask(pdAppInstance->i2cHandle);
}

void PD_AuthTaskFn(pd_app_t *pdAppInstance)
{
    ...
    if ((!pdAppInstance->authDone) && (pdAppInstance->selfSouceCapNumber > 1))
    {
        pdAppInstance->authDone = 1u;
