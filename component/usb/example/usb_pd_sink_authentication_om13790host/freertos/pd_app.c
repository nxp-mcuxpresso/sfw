...
SemaphoreHandle_t g_DemoSemaphore;
...
void HW_TimerCallback(void *callbackParam)
{
    ...
    for (index = 0; index < PD_DEMO_PORTS_COUNT; ++index)
    {
        ...
        {
            PD_I2cOverVdmTimer1msIsr(g_PDAppInstanceArray[index]->i2cOvdmHandle);
        }
    }
...
void PD_PortAuthI2cOverVdmTask(void *arg)
{
    while (1)
    {
...
static void pd_demo_task(void *arg)
{
...
#if (defined PD_CONFIG_COMMON_TASK) && (PD_CONFIG_COMMON_TASK)
    if (xTaskCreate(PD_PortsTask, "PD", (2048u + 512u) / sizeof(portSTACK_TYPE), NULL, 6, NULL) != pdPASS)
    {
    ...
    {
        if (xTaskCreate(PD_PortTask, "port", (2048u + 512u) / sizeof(portSTACK_TYPE), g_PDAppInstanceArray[index], 6,
                        NULL) != pdPASS)
        {
            PRINTF("create task error\r\n");
        }
    ...
    for (uint8_t index = 0; index < PD_DEMO_PORTS_COUNT; ++index)
    {
        if (xTaskCreate(PD_PortAuthI2cOverVdmTask, "i2c", 1024 / sizeof(portSTACK_TYPE),
                        g_PDAppInstanceArray[index]->i2cOvdmHandle, 5, NULL) != pdPASS)
        {
            PRINTF("create task error\r\n");
        }
    }
