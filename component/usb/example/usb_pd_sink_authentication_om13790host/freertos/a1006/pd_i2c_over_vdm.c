...
#define NXP_VDM_SENDER_RESPONSE (60)
...
static void PD_I2cOverVdmTimer_Stop(i2c_over_vdm_state_t *i2cOvdmState)
{
    uint32_t sr = DisableGlobalIRQ();
    __ASM("CPSID i");
    i2cOvdmState->timerValue = 0;
    EnableGlobalIRQ(sr);
    (void)xEventGroupClearBits(i2cOvdmState->taskEvent, I2C_OVER_VDM_EVENT_TIMER_OUT);
}
...
void PD_I2cOverVdmTask(void *arg)
{
    ...
    if (eventBits & I2C_OVER_VDM_EVENT_MSG_RECEIVED)
    {
        ...
        I2CoVDMPayLoad += 2;
        PD_I2cOverVdmTimer_Stop(i2cOvdmState);
                ...
                if (pkgi2cOverVdmHdr->curr_seq == pkgi2cOverVdmHdr->total_seq)
                {
                    ...
                    switch (pkgi2cOverVdmHdr->cmd)
                    {
                        case I2COVDM_CMD_READ_REQ:
                            ...
                            if (PD_I2cReadBlocking(i2cOvdmState->i2cHandle,
                                                   i2cOvdmState->i2cOvdmCommonBuf[I2C_VDM_SLAVEADDR_OFFSET],
                                                   (i2cOvdmState->i2cOvdmCommonBuf[I2C_VDM_CMDADDR_HB_OFFSET] << 8) |
                                                       (i2cOvdmState->i2cOvdmCommonBuf[I2C_VDM_CMDADDR_LB_OFFSET]),
                                                   2, i2cOvdmState->i2cOvdmCommonBuf, length) == kStatus_PD_Success)
                            {
                        ...
                        case I2COVDM_CMD_EXTENDED:
                            ...
                            /* prepare data */
                            if (PD_I2cWriteBlocking(i2cOvdmState->i2cHandle,
                                                    i2cOvdmState->i2cOvdmCommonBuf[I2C_VDM_WRITE_SLAVEADDR_OFFSET], 0,
                                                    0, &i2cOvdmState->i2cOvdmCommonBuf[I2C_VDM_WRITE_PAYLOAD_OFFSET],
                                                    length) == kStatus_PD_Success)
                            {
                                /* transmit success */
                                i2cOvdmState->i2cOvdmCommonBuf[0] = I2COVDM_STAT_ACK;
                            }
...
uint8_t PD_I2cOverVdmInit(pd_i2c_over_vdm_handle *i2cOvdmHandle, pd_i2c_over_vdm_config_t *config)
{
    ...
    PD_I2cInit(&s_i2cOverVdmState[index].i2cHandle, config->i2cInstance, config->i2cSrcClock,
               (PD_I2cReleaseBus)config->i2cReleaseBus);
...
uint8_t PD_I2cOverVdmRead(
    pd_i2c_over_vdm_handle i2cOvdmHandle, uint8_t slaveAddress, uint16_t offset, uint8_t *buff, uint8_t length)
{
    i2c_over_vdm_state_t *i2cOvdmState = (i2c_over_vdm_state_t *)i2cOvdmHandle;
    ...
    i2cOvdmState->i2cOvdmCommonBuf[1]   = slaveAddress;
                           ...
                           uint8_t length)
{
    ...
    i2cOvdmState->I2CoVDMTxBufEnd     = 0;
    i2cOvdmState->I2CoVDMCurrIndex    = 0;
    i2cOvdmState->i2cOvdmCommonBuf[0] = slaveAddress;
