...
uint32_t *g_PictureBuffer;
uint32_t g_PictureBufferDataLength = 0;
...
static void USB_HostVideoStreamDataInCallback(void *param, uint8_t *data, uint32_t dataLen, usb_status_t status)
{
    ...
    if (dataLen > 0)
    {
        if (dataLen > 11)
        {
            ...
            if (dataSize)
            {
                if (videoAppInstance->pictureBufferState &&
                    (!g_DiscardFrame)) /* the picture buffer are not available, now discard the receiving picture */
                {
                    g_DiscardFrame = 1;
                }
                else if (!g_DiscardFrame)
                {
                    if (dataSize > (videoAppInstance->videoCameraPictureBufferSize - g_PictureBufferDataLength))
                    {
                        dataSize = videoAppInstance->videoCameraPictureBufferSize - g_PictureBufferDataLength;
                    }
                    /* copy data to picture buffer */
                    memcpy((void *)((uint8_t *)&g_PictureBuffer[0] + g_PictureBufferDataLength),
                           (void *)(((uint8_t *)&g_StreamBuffer[videoAppInstance->streamBufferIndex][0]) + headLength),
                           dataSize),
                        OSA_ENTER_CRITICAL();
                    g_PictureBufferDataLength += dataSize;
                    OSA_EXIT_CRITICAL();
                }
                else
                {
            ...
            if (g_DiscardFrame) /* this picture is discarded */
            {
                ...
                {
                    if (g_PictureBufferDataLength != 0)
                    {
            ...
            else if (endOfFrame) /* can save this picture and get the last frame */
            {
                if (g_PictureBufferDataLength != 0)
                {
                    videoAppInstance->pictureBufferState = 1;
                }
                else
                {
                }
    ...
    USB_HosVideoStreamRecv(
        videoAppInstance->classHandle, (uint8_t *)&g_StreamBuffer[videoAppInstance->streamBufferIndex][0],
...
static void USB_HostVideoWriteSDCard(void *param)
{
    ...
    if (videoAppInstance->pictureBufferState)
    {
        ...
        fileStatus = f_write(&fileObj, (uint8_t *)&g_PictureBuffer[0], g_PictureBufferDataLength, (UINT *)&writeSize);
        if ((fileStatus != FR_OK) || (g_PictureBufferDataLength != writeSize))
        {
            usb_echo("sdcard operate fail\r\n");
        }

        OSA_ENTER_CRITICAL();
        videoAppInstance->pictureBufferState = 0;
        g_PictureBufferDataLength            = 0;
        OSA_EXIT_CRITICAL();
    ...
    switch (videoAppInstance->runState)
    {
        ...
        case kUSB_HostVideoRunGetSetProbeCommitDone:
        {
            ...
            if (videoAppInstance->cameraDeviceFormatType == USB_HOST_DESC_SUBTYPE_VS_FORMAT_MJPEG)
            {
                ...
                g_PictureBuffer = (uint32_t *)malloc(videoAppInstance->videoCameraPictureBufferSize);
        ...
        case kUSB_HostEventEnumerationDone:
            if (g_Video.devState == kStatus_DEV_Idle)
            {
                if ((g_VideoDeviceHandle != NULL) && (g_VideoControlInterfaceHandle != NULL))
                {
                    ...
                    g_Video.currentStreamInterfaceAlternate = 0;
