...
void USB_AudioRecorderFSSync()
{
    ...
    if (g_deviceComposite->audioUnified.recorderIntervalCount != AUDIO_CALCULATE_Ff_INTERVAL)
    {
        g_deviceComposite->audioUnified.recorderIntervalCount++;
        return;
    }
    g_deviceComposite->audioUnified.recorderIntervalCount = 1;

    usedSpace = USB_AudioRecorderBufferSpaceUsed();
    if (usedSpace >= AUDIO_BUFFER_UPPER_LIMIT(AUDIO_RECORDER_DATA_WHOLE_BUFFER_LENGTH * FS_ISO_OUT_ENDP_PACKET_SIZE))
    {
        audio_trim_down();
    }
    else if (usedSpace <
             AUDIO_BUFFER_LOWER_LIMIT(AUDIO_RECORDER_DATA_WHOLE_BUFFER_LENGTH * FS_ISO_OUT_ENDP_PACKET_SIZE))
    {
        audio_trim_up();
    }
    else
...
usb_status_t USB_DeviceAudioProcessTerminalRequest(uint32_t audioCommand, uint32_t *length, uint8_t **buffer, uint8_t entityOrEndpoint)
{
    ...
    switch (audioCommand)
    {
        ...
        case USB_DEVICE_AUDIO_SET_CUR_SAM_FREQ_CONTROL:
            g_deviceComposite->audioUnified.curSampleFrequency = **(buffer);
            break;
...
void USB_DeviceAudioSpeakerStatusReset(void)
{
    ...
    g_deviceComposite->audioUnified.recorderIntervalCount  = 0;
