...
static usb_status_t USB_DeviceCallback(usb_device_handle handle, uint32_t event, void *param)
{
    ...
    (defined(USB_DEVICE_CONFIG_LPCIP3511HS) && (USB_DEVICE_CONFIG_LPCIP3511HS > 0U))
        ...
        case kUSB_DeviceEventGetHidPhysicalDescriptor:
            ...
            break;
        case kUSB_DeviceEventSetBHNPEnable:
...
usb_status_t USB_OtgDeviceMouseInit(usb_device_handle *deviceHandle)
{
/*
 * If the SOC has USB KHCI dedicated RAM, the RAM memory needs to be clear after
 * the KHCI clock is enabled.
 */
    ...
    if (kStatus_USB_Success != USB_DeviceClassInit(kUSB_ControllerKhci0, &g_UsbDeviceHidConfigList, deviceHandle))
    {
    ...
    g_UsbDeviceHidMouse.deviceHandle = *deviceHandle;
    ...
    if (xTaskCreate(USB_DeviceKhciTaskFunction, "device stack task", 2000L / sizeof(portSTACK_TYPE), *deviceHandle, 5,
                    &(g_UsbDeviceHidMouse.deviceTaskHandler)) != pdPASS)
    {
...
usb_status_t USB_OtgDeviceMouseDeinit(usb_device_handle deviceHandle)
{
...
#if USB_DEVICE_CONFIG_USE_TASK
    vTaskDelete(g_UsbDeviceHidMouse.deviceTaskHandler);
#endif

    return USB_DeviceClassDeinit(kUSB_ControllerKhci0);
}
