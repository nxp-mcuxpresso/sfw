...
static void pd_demo_task(void *arg)
{
...
#if (defined PD_CONFIG_COMMON_TASK) && (PD_CONFIG_COMMON_TASK)
    if (xTaskCreate(PD_PortsTask, "PD", (2048u) / sizeof(portSTACK_TYPE), NULL, 6, NULL) != pdPASS)
    {
    ...
    {
        if (xTaskCreate(PD_PortTask, "port", (1024u + 512u) / sizeof(portSTACK_TYPE), g_PDAppInstanceArray[index], 6,
                        NULL) != pdPASS)
        {
            PRINTF("create task error\r\n");
        }
