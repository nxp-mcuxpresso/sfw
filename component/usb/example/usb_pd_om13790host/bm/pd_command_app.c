...
pd_status_t PD_DpmSnkRDOResultCallback(void *callbackParam, uint8_t success, uint32_t failResultTypeOrRDO)
{
    ...
    if (success)
    {
        ...
        PRINTF("sink request %dV success\r\n", (uint16_t)((vbusPower.minVoltage * 50) / 1000));
    }
    else
    {
        ...
        switch (svdmResult->vdmCommand)
        {
            ...
            case kVDM_DiscoverSVIDs:
            {
                ...
                for (index = 0; index < (svdmResult->vdoCount); ++index)
                {
                    ...
                    if (SVID != 0)
                    {
                        ...
                        {
                            PRINTF("SVID%d: %x\r\n", svidIndex, (uint16_t)pdAppInstance->partnerSVIDs[index * 2]);
                            svidIndex++;
                        }
                        ...
                        {
                            PRINTF("SVID%d: %x\r\n", svidIndex, (uint16_t)pdAppInstance->partnerSVIDs[index * 2 + 1]);
                            svidIndex++;
                        }
                    }
            ...
            case kVDM_DiscoverModes:
            {
                ...
                for (index = 0; index < (svdmResult->vdoCount); ++index)
                {
                    ...
                    buffPtr += 2;
                    PRINTF("Mode%d: %x\r\n", 1 + index * 2, (uint16_t)pdAppInstance->partnerModes[index * 2]);
                    pdAppInstance->partnerModes[index * 2 + 1] = USB_SHORT_FROM_LITTLE_ENDIAN_ADDRESS(buffPtr);
                    buffPtr += 2;
                    PRINTF("Mode%d: %x\r\n", 1 + index * 2 + 1, (uint16_t)pdAppInstance->partnerModes[index * 2 + 1]);
                }
