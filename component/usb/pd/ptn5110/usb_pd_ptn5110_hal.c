...
pd_status_t PDPTN5110_HalInit(pd_handle pdHandle)
{
    ...
    else
    {
        ...
        Reg_CacheWrite(pdInstance, VBUS, vbus_sink_disconnect_threshold, 0x008CU);
        Reg_BusWriteWord(pdInstance, vbus_sink_disconnect_threshold, 0x008CU);
    }
...
static void PDPTN5110_FaultStatusChangeCallback(pd_instance_t *pdInstance)
{
    ...
    Reg_BusWriteByte(pdInstance, fault_status, 0xFF);

    /* TODO: fault process */
    if ((faultStatus & TCPC_FAULT_STATUS_FORCE_DISCHARGE_FAILED_MASK) != 0U)
    {
        PDPTN5110_ConnectRawVbusDischarge(pdInstance, 0);
    }
