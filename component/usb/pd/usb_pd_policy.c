...
static void PD_PsmReset(pd_instance_t *pdInstance)
{
    ...
    pdInstance->sinkWaitRpAMS   = PD_DPM_INVALID;
#endif
...
static uint8_t PD_PsmCheckOnlyVbus(pd_instance_t *pdInstance)
{
    ...
    (void)PD_PhyControl(pdInstance, PD_PHY_GET_VBUS_POWER_STATE, &powerState);
    if ((powerState & PD_VBUS_POWER_STATE_VBUS_MASK) != 0U)
    {
        return 1;
    }
    return 0;
...
static void PD_PsmCheckFRS5V(pd_instance_t *pdInstance)
{
    if ((pdInstance->fr5VOpened == 0U) && (PD_PsmCheckLessOrEqualVsafe5v(pdInstance) != 0U))
    {
...
static uint8_t PD_PsmCheckVsafe0V(pd_instance_t *pdInstance)
{
    ...
    (void)PD_PhyControl(pdInstance, PD_PHY_GET_VBUS_POWER_STATE, &powerState);
    return (powerState & (uint8_t)PD_VBUS_POWER_STATE_VSAFE0V_MASK);
}
...
#endif
    {
        if ((pdInstance->pdPowerPortConfig->sourceCapCount != 0U) &&
        ...
        else if ((pdInstance->pdPowerPortConfig->sinkCapCount != 0U) &&
...
#if defined(PD_CONFIG_PD3_AMS_COLLISION_AVOID_ENABLE) && (PD_CONFIG_PD3_AMS_COLLISION_AVOID_ENABLE)
    uint8_t start = 1;
    typec_current_val_t typecCurrent;
        ...
        if (isInitiator != 0U)
        {
            ...
            else
            {
                ...
                if ((command != PD_DPM_CONTROL_HARD_RESET) && (command != PD_DPM_CONTROL_SOFT_RESET))
                {
                    ...
                    {
                        pdInstance->sinkWaitRpAMS = command;
                        start                     = 0;
                    }
                }
            }
...
#if (defined PD_CONFIG_PD3_FAST_ROLE_SWAP_ENABLE) && (PD_CONFIG_PD3_FAST_ROLE_SWAP_ENABLE)
        case PD_DPM_FAST_ROLE_SWAP:
            if (PD_PsmStartCommand(pdInstance, PD_DPM_FAST_ROLE_SWAP, 1) != 0U)
            {
                    ...
                    if (pdInstance->curPowerRole == kPD_PowerRoleSource)
                    {
                             ...
                             (uint8_t)kAutoRequestProcess_Accept))
                        {
                            if (PD_PsmSendControlTransitionWithAsyncRxAndSoftReset(
                                    pdInstance, kPD_MsgWait, PSM_PE_PRS_SRC_SNK_EVALUATE_PR_SWAP_GET_SOURCE_CAP) != 0U)
                            {
#if (defined PD_CONFIG_ENABLE_AUTO_POLICY) && (PD_CONFIG_ENABLE_AUTO_POLICY)
...
static void PD_PsmTimerWait(pd_instance_t *pdInstance, uint16_t time)
{
    (void)PD_TimerStart(pdInstance, tDelayTimer, time);
    while (PD_TimerCheckInvalidOrTimeOut(pdInstance, tDelayTimer) == 0U)
    {
        ;
    }
}
...
static inline void PD_PsmEnterSrcSendCapsState(pd_instance_t *pdInstance)
{
    ...
    if (pdInstance->commandProcessing == PD_DPM_INVALID)
    {
        ...
        if (PD_PsmStartCommand(pdInstance, PD_DPM_CONTROL_POWER_NEGOTIATION,
#if defined(PD_CONFIG_PD3_AMS_COLLISION_AVOID_ENABLE) && (PD_CONFIG_PD3_AMS_COLLISION_AVOID_ENABLE)
...
static inline void PD_PsmEnterSrcTransitionToDefaultState(pd_instance_t *pdInstance)
{
    ...
    do
    {
        ...
        delayTime = 10;
        while ((PD_PsmCheckVsafe0V(pdInstance) == 0U) && (delayTime != 0U))
        {
            delayTime--;
        }
    } while (delayTime == 0U);
...
static pd_state_machine_state_t PD_PsmEnterState(pd_instance_t *pdInstance)
{
    ...
    while (pdInstance->psmNewState != pdInstance->psmCurState)
    {
        ...
        switch (pdInstance->psmCurState)
        {
            ...
            case PSM_SOFT_RESET: /* (C) */
                ...
                {
                    (void)PD_DpmAppCallback(pdInstance, PD_DPM_SOFT_RESET_REQUEST, NULL, 0);
                }
                break;
    ...
    ((defined PD_CONFIG_SINK_ROLE_ENABLE) && (PD_CONFIG_SINK_ROLE_ENABLE))
            ...
            case PSM_PE_VCS_TURN_ON_VCONN: /* (C) */
                pdInstance->psmPresentlyVconnSource = kPD_IsVconnSource;
                /* Inform the DPM */
                (void)pdInstance->callbackFns->PD_ControlVconn(pdInstance->callbackParam, 1);
...
static pd_state_machine_state_t PD_PsmGetNewestEvent(pd_instance_t *pdInstance, psm_trigger_info_t *triggerInfo)
{
    ...
    {
        pd_command_t command;
        command             = (pd_command_t)PD_DpmGetMsg(pdInstance);
        ...
        if (triggerInfo->dpmMsg != PD_DPM_INVALID)
        {
...
static pd_state_machine_state_t PD_PsmProcessState(pd_instance_t *pdInstance)
{
    ...
    (defined(PD_CONFIG_SRC_AUTO_DISCOVER_CABLE_PLUG) && (PD_CONFIG_SRC_AUTO_DISCOVER_CABLE_PLUG))
                ...
                case PSM_PE_SRC_TRANSITION_TO_DEFAULT: /* (B) */
                {
                    ...
                    else if (PD_PsmCheckVbus5V(pdInstance) != 0U)
                    {
                        (void)PD_DpmAppCallback(pdInstance, PD_DPM_SRC_HARD_RESET_REQUEST, NULL, 1);
                        (void)PD_TimerStart(pdInstance, tNoResponseTimer, T_NO_RESPONSE);
                        pdInstance->enterSrcFromSwap = 0;
                        pdInstance->psmNewState      = PSM_PE_SRC_STARTUP;
                    }
                ...
                case PSM_PE_SNK_DISCOVERY: /* (B) */
                {
                    ...
                    {
                        if ((PD_TimerCheckInvalidOrTimeOut(pdInstance, tPSHardResetTimer) != 0U) ||
                            (PD_PsmCheckVsafe0V(pdInstance) != 0U))
                        {
                    ...
                    else if (PD_PsmCheckOnlyVbus(pdInstance) != 0U)
                    {
                        if (pdInstance->asmHardResetSnkProcessing != 0U)
                        {
                            ...
                            (void)pdInstance->callbackFns->PD_SnkDrawTypeCVbus(
                                ...
                                kVbusPower_InHardReset);
                            (void)PD_DpmAppCallback(pdInstance, PD_DPM_SNK_HARD_RESET_REQUEST, NULL, 1U);
                        }
                ...
                {
#if defined(PD_CONFIG_PD3_AMS_COLLISION_AVOID_ENABLE) && (PD_CONFIG_PD3_AMS_COLLISION_AVOID_ENABLE)
                    if ((pdInstance->sinkWaitRpAMS != PD_DPM_INVALID) && (triggerInfo.triggerEvent == PSM_TRIGGER_NONE))
                    {
                        typec_current_val_t typecCurrent;
                        ...
                        if (typecCurrent == kCurrent_3A)
                        {
                            triggerInfo.dpmMsg        = pdInstance->sinkWaitRpAMS;
                            triggerInfo.triggerEvent  = PSM_TRIGGER_DPM_MSG;
                            pdInstance->sinkWaitRpAMS = PD_DPM_INVALID;
                        }
                    }
                    ...
                    if (triggerInfo.triggerEvent == PSM_TRIGGER_PD_MSG)
                    {
                                                                     ...
                                                                     (triggerInfo.pdMsgType == kPD_MsgNotSupported)))
                        {
...
#endif
                            {
                                ...
                                (void)PD_DpmAppCallback(pdInstance, PD_DPM_GET_PARTNER_SRC_CAP_FAIL,
                ...
                case PE_FRS_SRC_SNK_Transition_to_off: /* B */
                {
                    ...
                    if ((PD_TimerCheckInvalidOrTimeOut(pdInstance, timrFRSwapWaitPowerStable) != 0U) &&
                        (PD_PsmCheckOnlyVbus(pdInstance) != 0U))
                    {
                ...
                {
                    if (PD_PsmCheckVbus5V(pdInstance) != 0U)
                    {
                                          ...
                                          psm_trigger_info_t *triggerInfo)
{
    ...
    if (triggerInfo->pdMsgType == kPD_MsgVendorDefined)
    {
                ...
                switch ((pd_vdm_command_t)triggerInfo->vdmHeader.bitFields.command)
                {
                    case kVDM_DiscoverIdentity:
                        if (triggerInfo->vdmHeader.bitFields.SVID == PD_STANDARD_ID)
                        {
                            ...
                            {
                                uint8_t index;
                                for (index = 0; index < commandVdmRequest.vdoCount; ++index)
                                {
                                    pdInstance->sendingData[index + 2U] = commandVdmRequest.vdoData[index];
                                }
                                if (pdInstance->revision < PD_SPEC_REVISION_30)
                                {
                                    ((pd_id_header_vdo_t *)((void *)&pdInstance->sendingData[2]))
                                        ->bitFields.productTypeDFP = 0;
                                }
                            }
                        }
                        break;
        ...
        else
        {
            ...
            (void)PD_DpmAppCallback(pdInstance, PD_DPM_UNSTRUCTURED_VDM_RECEIVED, &unstructuredVDMParam, 1);
    ...
    if (connectState != kConnectState_NotStable)
    {
        ...
        else if (pdInstance->isConnected != 0U) /* connect result stable and connected */
        {
            ...
            else
            {
                ...
                if (PD_PsmCheckOnlyVbus(pdInstance) != 0U)
                {
...
void PD_DpmSetVconn(pd_instance_t *pdInstance, uint8_t enable)
{
    if (pdInstance->pdPowerPortConfig->vconnSupported != 0U)
    {
        (void)PD_PhyControl(pdInstance, PD_PHY_CONTROL_VCONN, &enable);
    }
}
#endif
...
pd_status_t PD_Control(pd_handle pdHandle, uint32_t controlCode, void *param)
{
    ...
    switch ((pd_control_t)controlCode)
    {
        case PD_CONTROL_GET_PD_STATE:
            /* Timer busy, Event pending, Command Process, Not ready && Not SrcDisabled && not SnkWaitCap && Attached */
            /* 1. Timer runing, event pending or command process indicates PD state machine is busy. */
            /* 2. Ready state indicates PD stack has completed PD negotiation. */
            /* 3. SrcDisabled state indicates partner sink doesn't reply GoodCRC in response to SrcCapability message.
                  There isn't PD establishtment or partner sink doesn't support PD communication. */
            /* 4. SnkWaitCap state indicates self sink doesn't receive SrcCapability message.
                  There isn't PD establishtment or partner source doesn't support PD communication. */
        ...
        {
            uint32_t tmp32Val = 1;
            (void)PD_PhyControl(pdInstance, PD_PHY_DISCHARGE_VBUS, &tmp32Val);
