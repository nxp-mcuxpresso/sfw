                                         ...
                                         hal_i2c_direction_t direction)
{
    ...
    if (
#if defined(FSL_RTOS_FREE_RTOS)
        (PD_CheckInterruptContext() == 0U) &&
#endif
        (OSA_MutexLock(&i2cAdapter->i2cMutex[0], osaWaitForever_c) != KOSA_StatusSuccess))
    {
        return kStatus_PD_Error;
    }
    ...
    if (
        ...
        (PD_CheckInterruptContext() == 0U) &&
#endif
        (OSA_MutexUnlock(&i2cAdapter->i2cMutex[0]) != KOSA_StatusSuccess))
    {
        return kStatus_PD_Error;
    }
