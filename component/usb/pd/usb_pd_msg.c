...
static pd_status_t PD_MsgWaitSendResult(pd_instance_t *pdInstance);
...
static pd_status_t PD_MsgWaitSendResult(pd_instance_t *pdInstance)
{
    ...
    (void)PD_TimerStart(pdInstance, timrMsgSendWaitResultTimer, T_WAIT_SEND_RESULT);
    do
    {
        (void)OSA_EventWait(pdInstance->taskEventHandle, PD_TASK_EVENT_ALL, 0, 5, &eventSet);
        PD_PortTaskEventProcess(pdInstance, eventSet);
        if ((eventSet & (uint32_t)PD_TASK_EVENT_SEND_DONE) != 0U)
        {
            (void)PD_TimerClear(pdInstance, timrMsgSendWaitResultTimer);
            (void)OSA_EventClear(pdInstance->taskEventHandle, PD_TASK_EVENT_SEND_DONE);
            break;
        }

        /* Disconnected */
        if ((PD_TimerCheckInvalidOrTimeOut(pdInstance, timrMsgSendWaitResultTimer) != 0U) ||
            (PD_ConnectCheck(pdInstance) == kConnectState_Disconnected))
        {
            (void)PD_TimerClear(pdInstance, timrMsgSendWaitResultTimer);
            pdInstance->sendingState = 0;
    ...
    pd_instance_t *pdInstance, start_of_packet_t sop, message_type_t msgType, uint32_t dataLength, uint8_t *dataBuffer)
{
    ...
    {
        if (PD_MsgWaitSendResult(pdInstance) == kStatus_PD_Success)
        {
            return kStatus_PD_Success;
        }
    }
...
void PD_MsgSendHardOrCableReset(pd_instance_t *pdInstance, uint8_t hardResetOrCableReset)
{
    ...
    (void)PD_TimerStart(pdInstance, tMsgHardResetCompleteTimer, T_HARD_RESET_COMPLETE);
    ...
    do
    {
        (void)OSA_EventWait(pdInstance->taskEventHandle, PD_TASK_EVENT_ALL, 0, 5, &eventSet);
        PD_PortTaskEventProcess(pdInstance, eventSet);
        if ((PD_TimerCheckInvalidOrTimeOut(pdInstance, tMsgHardResetCompleteTimer) != 0U) ||
            ((eventSet & (uint32_t)PD_TASK_EVENT_SEND_DONE) != 0U))
        {
            (void)PD_TimerClear(pdInstance, tMsgHardResetCompleteTimer);
            (void)OSA_EventClear(pdInstance->taskEventHandle, PD_TASK_EVENT_SEND_DONE);
            pdInstance->sendingState = 0;
...
void PD_MsgSrcStartCommand(pd_instance_t *pdInstance)
{
    ...
    if (pdInstance->revision >= PD_SPEC_REVISION_30)
    {
        if (pdInstance->commandSrcOwner == 0U)
        {
            ...
            (void)PD_TimerStart(pdInstance, tSinkTxTimer, T_SINK_TX);
        }

        if (PD_TimerCheckInvalidOrTimeOut(pdInstance, tSinkTxTimer) == 0U)
        {
            do
            {
                uint32_t eventSet = 0;
                ...
                PD_PortTaskEventProcess(pdInstance, eventSet);
                if ((PD_TimerCheckInvalidOrTimeOut(pdInstance, tSinkTxTimer) != 0U) &&
                    ((eventSet & (uint32_t)PD_TASK_EVENT_TIME_OUT) != 0U))
                {
                    (void)PD_TimerClear(pdInstance, tSinkTxTimer);
                    /* (void)OSA_EventClear(pdInstance->taskEventHandle, PD_TASK_EVENT_TIME_OUT); */
                    break;
                }

                if ((eventSet & ((uint32_t)PD_TASK_EVENT_RECEIVED_HARD_RESET | (uint32_t)PD_TASK_EVENT_PD_MSG |
                                 (uint32_t)PD_TASK_EVENT_DPM_MSG | (uint32_t)PD_TASK_EVENT_FR_SWAP_SINGAL)) != 0U)
                {
                    return;
                }
    ...
    if (pdInstance->revision >= PD_SPEC_REVISION_30)
    {
        ...
        if (typecCurrent != kCurrent_3A)
        {
            do
            {
                ...
                (void)OSA_EventWait(pdInstance->taskEventHandle, 0xffffu, 0, 5, &eventSet);
                PD_PortTaskEventProcess(pdInstance, eventSet);
                (void)PD_PhyControl(pdInstance, PD_PHY_GET_TYPEC_CURRENT_CAP, &typecCurrent);
                if (typecCurrent == kCurrent_3A)
                {
                ...
                if ((eventSet & (PD_TASK_EVENT_RECEIVED_HARD_RESET | PD_TASK_EVENT_PD_MSG |
                                 PD_TASK_EVENT_DPM_MSG | PD_TASK_EVENT_FR_SWAP_SINGAL)))
                {
                    return 0;
                }
