...
def update_for_board()
    if ARGV.include?("-b")
        ...
        if param.match(/frdmk66f/)
            $BOARD = 'frdmk66f'
            if File.directory?("#{$root_dir}") and File.directory?("#{$root_dir}/boards/#{$BOARD}/lwip_examples")
                update_config_file_for_frdmk66f("#{$root_dir}/boards/#{$BOARD}/lwip_examples")
                # puts "The config file for #{$BOARD} is updated!"
            else
                puts "Can't open the file #{$root_dir}/boards/#{$BOARD}/lwip_examples"
            end
        end
        if param.match(/evkmcimx6ul/)
            $BOARD = 'evkmcimx6ul'
            if File.directory?("#{$root_dir}") and File.directory?("#{$root_dir}/boards/#{$BOARD}/lwip_examples")
                update_config_file_for_evkmcimx6ul("#{$root_dir}/boards/#{$BOARD}/lwip_examples")
                # puts "The config file for #{$BOARD} is updated!"
            else
                puts "Can't open the file #{$root_dir}/boards/#{$BOARD}/lwip_examples"
            end
        ...
        if param.match(/evkmcimx6ull/)
            $BOARD = 'evkmcimx6ull'
            if File.directory?("#{$root_dir}") and File.directory?("#{$root_dir}/boards/#{$BOARD}/lwip_examples")
                update_config_file_for_evkmcimx6ull("#{$root_dir}/boards/#{$BOARD}/lwip_examples")
                # puts "The config file for #{$BOARD} is updated!"
            else
                puts "Can't open the file #{$root_dir}/boards/#{$BOARD}/lwip_examples"
            end
        ...
        if param.match(/evk_k32h844p/)
            $BOARD = 'evk_k32h844p'
            if File.directory?("#{$root_dir}") and File.directory?("#{$root_dir}/boards/#{$BOARD}/lwip_examples")
                update_config_file_for_evk_k32h844p("#{$root_dir}/boards/#{$BOARD}/lwip_examples")
                # puts "The config file for #{$BOARD} is updated!"
            else
                puts "Can't open the file #{$root_dir}/boards/#{$BOARD}/lwip_examples"
            end
        ...
        if param.match(/evkmimxrt1050/)
            $BOARD = 'evkmimxrt1050'
            if File.directory?("#{$root_dir}") and File.directory?("#{$root_dir}/boards/#{$BOARD}/lwip_examples")
                update_config_file_for_evkmimxrt1050("#{$root_dir}/boards/#{$BOARD}/lwip_examples")
                # puts "The config file for #{$BOARD} is updated!"
            else
                puts "Can't open the file #{$root_dir}/boards/#{$BOARD}/lwip_examples"
            end
        ...
        if param.match(/evkmimxrt1020/)
            $BOARD = 'evkmimxrt1020'
            if File.directory?("#{$root_dir}") and File.directory?("#{$root_dir}/boards/#{$BOARD}/lwip_examples")
                update_config_file_for_evkmimxrt1020("#{$root_dir}/boards/#{$BOARD}/lwip_examples")
                # puts "The config file for #{$BOARD} is updated!"
            else
                puts "Can't open the file #{$root_dir}/boards/#{$BOARD}/lwip_examples"
            end
        ...
        if param.match(/evkmimxrt1024/)
            $BOARD = 'evkmimxrt1024'
            if File.directory?("#{$root_dir}") and File.directory?("#{$root_dir}/boards/#{$BOARD}/lwip_examples")
                update_config_file_for_evkmimxrt1024("#{$root_dir}/boards/#{$BOARD}/lwip_examples")
                # puts "The config file for #{$BOARD} is updated!"
            else
                puts "Can't open the file #{$root_dir}/boards/#{$BOARD}/lwip_examples"
            end
        ...
        if param.match(/evkbimxrt1050/)
            $BOARD = 'evkbimxrt1050'
            if File.directory?("#{$root_dir}") and File.directory?("#{$root_dir}/boards/#{$BOARD}/lwip_examples")
                update_config_file_for_evkbimxrt1050("#{$root_dir}/boards/#{$BOARD}/lwip_examples")
                # puts "The config file for #{$BOARD} is updated!"
            else
                puts "Can't open the file #{$root_dir}/boards/#{$BOARD}/lwip_examples"
            end
        ...
        if param.match(/evkmimxrt1060/)
            $BOARD = 'evkmimxrt1060'
            if File.directory?("#{$root_dir}") and File.directory?("#{$root_dir}/boards/#{$BOARD}/lwip_examples")
                update_config_file_for_evkmimxrt1060("#{$root_dir}/boards/#{$BOARD}/lwip_examples")
                # puts "The config file for #{$BOARD} is updated!"
            else
                puts "Can't open the file #{$root_dir}/boards/#{$BOARD}/lwip_examples"
            end
        ...
        if param.match(/lpcxpresso54608/)
            ...
            if File.directory?("#{$root_dir}") and File.directory?("#{$root_dir}/boards/#{$BOARD}/lwip_examples")
                update_config_file_for_lpcxpresso54608("#{$root_dir}/boards/#{$BOARD}/lwip_examples")
                # puts "The config file for #{$BOARD} is updated!"
            else
                puts "Can't open the file #{$root_dir}/boards/#{$BOARD}/lwip_examples"
            end
        ...
        if param.match(/lpcxpresso54618/)
            ...
            if File.directory?("#{$root_dir}") and File.directory?("#{$root_dir}/boards/#{$BOARD}/lwip_examples")
                update_config_file_for_lpcxpresso54618("#{$root_dir}/boards/#{$BOARD}/lwip_examples")
                # puts "The config file for #{$BOARD} is updated!"
            else
                puts "Can't open the file #{$root_dir}/boards/#{$BOARD}/lwip_examples"
            end
        ...
        if param.match(/lpcxpresso54s608/)
            ...
            if File.directory?("#{$root_dir}") and File.directory?("#{$root_dir}/boards/#{$BOARD}/lwip_examples")
                update_config_file_for_lpcxpresso54s608("#{$root_dir}/boards/#{$BOARD}/lwip_examples")
                # puts "The config file for #{$BOARD} is updated!"
            else
                puts "Can't open the file #{$root_dir}/boards/#{$BOARD}/lwip_examples"
            end
        ...
        if param.match(/lpcxpresso54628/)
            ...
            if File.directory?("#{$root_dir}") and File.directory?("#{$root_dir}/boards/#{$BOARD}/lwip_examples")
                update_config_file_for_lpcxpresso54628("#{$root_dir}/boards/#{$BOARD}/lwip_examples")
                # puts "The config file for #{$BOARD} is updated!"
            else
                puts "Can't open the file #{$root_dir}/boards/#{$BOARD}/lwip_examples"
            end
        ...
        if param.match(/lpcxpresso54018/)
            ...
            if File.directory?("#{$root_dir}") and File.directory?("#{$root_dir}/boards/#{$BOARD}/lwip_examples")
                update_config_file_for_lpcxpresso54018("#{$root_dir}/boards/#{$BOARD}/lwip_examples")
                # puts "The config file for #{$BOARD} is updated!"
            else
                puts "Can't open the file #{$root_dir}/boards/#{$BOARD}/lwip_examples"
            end
            $KIT = 'lpcxpresso54018_om13585'
            if File.directory?("#{$root_dir}") and File.directory?("#{$root_dir}/boards/#{$KIT}/lwip_examples")
                update_pd_config_file_for_lpcxpresso54018_om13585("#{$root_dir}/boards/#{$KIT}/lwip_examples")
            else
                puts "Can't open the file #{$root_dir}/boards/#{$KIT}/lwip_examples"
            end
        end
        ...
        if param.match(/lpc54018iotmodule/)
            ...
            if File.directory?("#{$root_dir}") and File.directory?("#{$root_dir}/boards/#{$BOARD}/lwip_examples")
                update_config_file_for_lpc54018iotmodule("#{$root_dir}/boards/#{$BOARD}/lwip_examples")
                # puts "The config file for #{$BOARD} is updated!"
            else
                puts "Can't open the file #{$root_dir}/boards/#{$BOARD}/lwip_examples"
            end
        ...
        if param.match(/lpc54s018iotmodule/)
            ...
            if File.directory?("#{$root_dir}") and File.directory?("#{$root_dir}/boards/#{$BOARD}/lwip_examples")
                update_config_file_for_lpc54s018iotmodule("#{$root_dir}/boards/#{$BOARD}/lwip_examples")
                # puts "The config file for #{$BOARD} is updated!"
            else
                puts "Can't open the file #{$root_dir}/boards/#{$BOARD}/lwip_examples"
            end
        ...
        if param.match(/qn908x/)
            ...
            if File.directory?("#{$root_dir}") and File.directory?("#{$root_dir}/boards/#{$BOARD}/lwip_examples")
                update_config_file_for_qn908xdk("#{$root_dir}/boards/#{$BOARD}/lwip_examples")
                # puts "The config file for #{$BOARD} is updated!"
            else
                puts "Can't open the file #{$root_dir}/boards/#{$BOARD}/lwip_examples"
            end
